// we have to count total number of ways to reach the given target with infinite number of denomiantions
...
int solve(vector<int> &arr, int tar, int index)
{
    ...
    int take = solve(arr, tar - arr[index], index);
...
int solve(vector<int> &arr, int tar, int index, vector<vector<int>> &dp)
{
    ...
    if (dp[index][tar] != -1)
        return dp[index][tar];
    int take = solve(arr, tar - arr[index], index, dp);
